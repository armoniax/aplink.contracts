...
#define NOTIFY_REWARD(predator, victim, quantity) \
    {	token::notifyreward_action act{ _self, { {_self, active_permission} } };\
...
namespace aplink {
    ...
    */
   class [[eosio::contract("aplink.token")]] token : public contract {
      public:
         ...
         **/
         ...
         static asset get_sum_balance( const name& token_contract_account, const name& owner, const symbol_code& sym_code )
         {
            accounts accountstable( token_contract_account, owner.value );
            const auto& ac = accountstable.get( sym_code.raw() );
            return ac.sum_balance;
         }
