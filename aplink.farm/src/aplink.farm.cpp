...
void farm::lease(const name& farmer, 
    ...
    CHECKC(title.size() < MAX_CONTENT_SIZE, err::CONTENT_LENGTH_INVALID, "title size too large, respect " + to_string(MAX_CONTENT_SIZE));
    CHECKC(uri.size() < MAX_CONTENT_SIZE, err::CONTENT_LENGTH_INVALID, "url size too large, respect " + to_string(MAX_CONTENT_SIZE));
